...
*Still need to put this in for Acropora*
...
# Read in the profile data
profiles_raw_iso <- read_tsv("profiles.abs.iso.txt", skip = 6) %>%
    select(sample_name = `...2`, `A1-A1qd-A1qe`:`G3fd`) %>%
    filter(!is.na(sample_name)) %>%
  select(sample_name, `A1-A1qd-A1qe`:`G3fd`)
...
dist_pver_ordered <- as.dist(as.matrix(dist_pver)[rownames(meta_pver), rownames(meta_pver)])
