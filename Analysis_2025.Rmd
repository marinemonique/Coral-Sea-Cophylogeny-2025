...
##Create distance matrix for symbioants
### 4.1.1 Montipora Distance Matrix
...
### 4.1.3 Acropora Symbioant Distance Matrix
...
load("~/AIMS@JCU/CoralSeaSymbioants/Coral-Sea-Cophylogeny-2025/kdist_monti_symbioant.RData")
# GUniFrac with alpha 0.5 (i.e Generalised UniFrac Distance)
du_monti_symbioant <- k_unidist_monti[, , "d_0.5"]   
...
load("~/AIMS@JCU/CoralSeaSymbioants/Coral-Sea-Cophylogeny-2025/ind_coords_acro.RData")
...
seq_data_acro %>% 
  distinct(sample_name) %>% 
  nrow()
 #should be 354 in the symbioant
...
#Create a matrix with only common sample names for symbioant
seqs_wide_symbioant_acro <- seq_data_acro %>%
  select(sample_name, name, value) %>%
...
k_unidist_symbioant_acro <- GUniFrac(seqs_wide_symbioant_acro, k_tree_acro_symbioant)   #GUniFrac calculates all the distances 
k_unidist_symbioant_acro <- k_unidist_symbioant_acro$unifracs


 # GUniFrac with alpha 0.5 (i.e Generalised UniFrac Distance)
du_acro_symbioant <- k_unidist_symbioant_acro[, , "d_0.5"]   
dist_acro_symbioant <- as.dist(du_acro_symbioant, diag = FALSE)
```
