...
P.dist_intersect_iso <- P.dist[intersect_samples_row_iso, intersect_samples_col_iso]
H.dist_intersect_iso <- H.dist[intersect_samples_row_iso, intersect_samples_col_iso ]
...
```{r}
load("~/AIMS@JCU/CoralSeaSymbioants/ITS2_montipora_CS_2024/montipora/monti_gen.RData") #load host data
...
k_unidist_symbioant <- GUniFrac(seqs_wide_symbioant, k_tree_monti_symbioant)   #GUniFrac calculates all the distances 
k_unidist_symbioant <- k_unidist_symbioant$unifracs
...
```{r}
load("~/AIMS@JCU/CoralSeaSymbioants/ITS2_acropora_CS_2024/acropora/acro_gen.RData") #load host data
...
# Subset the genind object to retain only matching individuals
acro_gen_subset <- acro_gen[ind_names_host %in% shared_seqs_acro$sample_name, ]
...
paco.acro <- procrustes(H.dist, P.dist, symmetric = FALSE)
summary(paco.acro)
